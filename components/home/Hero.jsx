    ...
    <section className="relative w-full h-screen overflow-hidden">
        ...
        <div className="hidden md:block absolute inset-0 z-0">
          {slides.map((slide, i) => (
            ...
            >
              <div className={`absolute inset-0 ${i === activeIndex ? 'bg-kenburns' : ''}`}>
                <Image
                  src={slide.bg}
                  alt={slide.title}
                  fill
        ...
        <div className="hidden md:flex relative z-10 h-full pt-16">
          ...
          {/* Left: Text */}
          <div className={`flex flex-col justify-center w-[42%] pl-12 xl:pl-20 2xl:pl-28 pr-6 text-white ${mounted ? 'mount-bg' : 'opacity-0'}`}>
            ...
            {/* Divider line */}
            <div key={`div-${textKey}`} className="hero-divider w-16 h-0.5 mb-5" style={{ background: '#785d32' }} />

            {/* Subtitle */}
            <p key={`sub-${textKey}`} className="hero-sub text-gray-300 mb-10 max-w-sm leading-relaxed text-base">
              {slides[activeIndex].subtitle}
            </p>

            {/* Buttons */}
            <div key={`btns-${textKey}`} className="hero-btns flex flex-wrap gap-4">
              <button
              ...
              <button className="btn-learn px-8 py-3.5 rounded-lg font-semibold border-2 border-white text-white transition-all duration-300 hover:scale-[1.03]">
                <span>Learn More</span>
              </button>
            </div>

            {/* Slide counter */}
            <div key={`counter-${textKey}`} className="hero-label mt-14 flex items-center gap-3">
              <span className="text-2xl font-bold tabular-nums" style={{ color: '#e8d9c4', fontFamily: 'var(--font-playfair)' }}>
                0{activeIndex + 1}
              </span>
              <span className="text-gray-500 text-sm">/ 0{slides.length}</span>
            </div>
          </div>

          {/* Right: Card slider */}
          <div className={`flex-1 h-full flex items-center overflow-hidden ${mounted ? 'mount-cards' : 'opacity-0'}`}>
            <Swiper
              ref={swiperRef}
              ...
              {slides.map((slide, i) => (
                <SwiperSlide key={i} className="hero-card-slide">
                  <div className="card-float w-full h-full relative">
                    ...
                    >
                      <p className="text-white font-semibold text-sm tracking-wide" style={{ fontFamily: 'var(--font-playfair)' }}>
                        {slide.title}
                      </p>
                    </div>
                  </div>
        ...
        <div className={`hero-pagination hidden md:flex absolute bottom-7 left-1/2 -translate-x-1/2 z-20 gap-2 items-center ${mounted ? 'mount-nav' : 'opacity-0'}`} />
            ...
            {slides.map((slide, i) => (
              <SwiperSlide key={i} className="relative">
                ...
                <div className="absolute inset-0 z-10 flex flex-col justify-center items-center text-center text-white px-6 pt-16 pb-14">
                  <span className="text-xs font-semibold tracking-[0.3em] uppercase mb-4 inline-flex items-center gap-2" style={{ color: '#e8d9c4' }}>
                    <span className="inline-block w-5 h-px bg-current opacity-60" />
                    Welcome to Talukdar
                  </span>
                  <h1 className="font-bold leading-tight mb-3" style={{ fontFamily: 'var(--font-playfair)', fontSize: 'clamp(2rem, 9vw, 3rem)' }}>
                    {slide.title}
                  </h1>
                  <div className="w-10 h-0.5 mb-4 mx-auto" style={{ background: '#785d32' }} />
                  <p className="text-gray-300 mb-8 text-sm max-w-xs leading-relaxed">{slide.subtitle}</p>
                  <div className="flex gap-3 flex-wrap justify-center">
                    <button className="btn-shop px-6 py-3 rounded-lg font-semibold text-white flex items-center gap-2 text-sm" style={{ backgroundColor: '#785d32' }}>
                      <span className="btn-shimmer" />
