      ...
      <div className="flex items-stretch h-full">
        ...
        <div className="flex min-w-0 flex-1 flex-col justify-between gap-3 p-3 sm:gap-4 sm:p-5">
          <div className="min-w-0">
            <h3 className="line-clamp-2 text-sm font-semibold text-slate-900 dark:text-brand-pale sm:text-lg">{p?.name}</h3>
            <p className="mt-1 line-clamp-2 text-xs text-slate-600 dark:text-[#9fa8cc] sm:mt-2 sm:text-sm" dangerouslySetInnerHTML={{ __html: p?.short_description }} />
          </div>

          <div className="flex items-center justify-between gap-2">
            <p className="text-base font-bold text-brand-navy dark:text-[#f0ebe3] sm:text-2xl">${p?.base_price}</p>
            <button
              type="button"
